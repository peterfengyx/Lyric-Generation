...
def train_val(model_type,
    ...
    noise_un_variable = cudalize(Variable(torch.randn(torch.Size([batch_size, 128])))) # torch.Size([10, 512])
    # normalize
    ...
    for line_num in range(max_line_number):
        ...
        se_input = softmax(sg_word_tensor) # torch.Size([10, 19495])
        ...
        for line_idx in range(1, max_line_length):
            ...
            se_input = sg_output_softmax # torch.Size([10, 19495])
            _, se_hidden = sentence_encoder(se_input, se_hidden, batch_size)
        ...
        le_input = line_latent_variable # torch.Size([10, 10261])
    ...
    predicted_genre = lyric_discriminator(generated_lyric_latent_variable)
    predicted_genre_softmax = softmax(predicted_genre)
    _, topi = predicted_genre_softmax.data.topk(1)

    correct_num = (topi.view(-1) == cudalize(genre_tensor)).sum().item()

    return 0.0, 0.0, correct_num/batch_size
...
def trainEpochs(sentence_encoder, 
    ...
    val_loader = data_utils.DataLoader(dataset=LyricDataset(train_set[test_idx_150_1]),
...
if __name__=='__main__':
    ...
    saving_dir_dis = "lyric_classifier_150_save"
    epoch_num_dis = 14
    ...
    lyric_discriminator = LyricClassifier(lc_input_size, lc_output_size)
    lyric_discriminator.load_state_dict(torch.load(saving_dir_dis+'/lyric_generator_'+str(epoch_num_dis)))
