...
def train_val(model_type,
    ...
    lg_temp_variable = noise_variable.detach()
    ...
    for line_num in range(max_line_number):
        ...
        lg_temp_variable = sg_hiddens_variable[sg_outputs_length, np.arange(batch_size), :].detach()
    ...
    # GAN starts
    y_real_ = cudalize(torch.ones(batch_size, 1))
    y_fake_ = cudalize(torch.zeros(batch_size, 1))

    D_result_real, C_result_real = lyric_discriminator(real_lyric_latent_variable)
    D_real_loss = BCE_loss(D_result_real, y_real_)
    C_real_loss = CE_loss(C_result_real, cudalize(genre_tensor))

    D_result_fake, C_result_fake = lyric_discriminator(generated_lyric_latent_variable)
    D_fake_loss = BCE_loss(D_result_fake, y_fake_)
    C_fake_loss = CE_loss(C_result_fake, cudalize(genre_tensor))
    ...
    # some problem here
    D_result_generate, C_result_generate = lyric_discriminator(generated_lyric_latent_variable)
    generator_loss = BCE_loss(D_result_generate, y_real_) + CE_loss(C_result_generate, cudalize(genre_tensor))
...
def trainEpochs(sentence_encoder, 
    ...
    BCE_loss = cudalize(nn.BCELoss())
    CE_loss = cudalize(nn.CrossEntropyLoss())
...
if __name__=='__main__':
    ...
    ldis_class_num = GenreSize
    lyric_discriminator = LyricDiscriminatorACGAN(ldis_input_size, ldis_class_num)
